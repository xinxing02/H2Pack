...
int  DAG_task_queue_get_task(DAG_task_queue_p tq)
{
    ...
    // Use ACQUIRE semantics to pair with RELEASE store in finish_task
    int task_id = __atomic_load_n(&tq->task_queue[task_head], __ATOMIC_ACQUIRE);
    while (task_id == -1)
    {
        //usleep(10);
        // Keep using ACQUIRE to establish synchronization when task becomes available
        task_id = __atomic_load_n(&tq->task_queue[task_head], __ATOMIC_ACQUIRE);
    }
    //if (task_id == -1) printf("[Warning] task_head = %d, task_id = -1\n", task_head);

    // Additional ACQUIRE fence after loading to ensure all prior writes are visible
    // This guarantees visibility of all J[], U[], J_coord[] writes from dependencies
...
void DAG_task_queue_finish_task(DAG_task_queue_p tq, const int task_id)
{
    ...
    for (int j = tq->DAG_src_ptr[task_id]; j < tq->DAG_src_ptr[task_id + 1]; j++)
    {
        ...
        if (dst_indeg == 0)
        {
            // CRITICAL: Release fence BEFORE making task visible
            // This ensures all writes from this task (and its dependencies)
            // are visible before we publish the new task
            __atomic_thread_fence(__ATOMIC_RELEASE);

            int task_tail = __atomic_fetch_add(&tq->task_tail, 1, __ATOMIC_SEQ_CST);

            // Store with RELEASE semantics to pair with ACQUIRE load in get_task
            // This creates a synchronizes-with relationship
            __atomic_store_n(&tq->task_queue[task_tail], dst, __ATOMIC_RELEASE);
        }
